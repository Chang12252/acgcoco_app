			...
			tabItemStyle() {
				return (index) => {
					let style = {
						...
						fontSize: this.tabsInfo.length > 0 ? (this.tabsInfo[index] ? this.tabsInfo[index].fontSize  + 'rpx' : this.fontSize  + 'rpx') : this.unFontSize + 'rpx',
				...
				return new Promise((resolve, reject) => {
					...
					view.exec(res => {
						...
						for (let i = 0; i < res.length; i++) {
							...
							res[i].fontSize = this.unFontSize;
							// 当前tab直接赋予activeColor
							if (i == this.getCurrent) res[i].color = this.activeColor;
							if (i == this.getCurrent) res[i].fontSize = this.fontSize;
							arr.push(res[i]);
						}
						this.tabsInfo = arr;
						resolve();
			...
			setFinishCurrent(current) {
				...
				if (current != this.animationFinishCurrent) {
					this.tabsInfo.map((val, index) => {
						...
						if (current == index) val.fontSize = this.fontSize;
						else val.fontSize = this.unFontSize;
						return val;	
					});
				}
				this.line3AddDx = 0;
