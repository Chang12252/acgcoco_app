...
<template>
	<scroll-view v-if="tabs.length" :id="viewId" class="me-tabs" :class="{'tabs-fixed': fixed}" :scroll-left="scrollLeft" scroll-x scroll-with-animation>
		<view class="tabs-item" :class="{'tabs-flex':!isScroll, 'tabs-scroll':isScroll}">
			<!-- tab -->
			<view class="tab-item" :style="{width: tabWidthVal}" v-for="(tab, i) in tabs" :class="{'active': value===i}" :key="i" @click="tabClick(i)">
				{{getTabName(tab)}}
			</view>
			<!-- 下划线 -->
...
<style lang="scss">
	.me-tabs{
		...
		&.tabs-fixed{
			z-index: 9999;
			position: fixed;
			top: calc(var(--status-bar-height) + 71px);
			left: 0;
			width: 100%;
		}
